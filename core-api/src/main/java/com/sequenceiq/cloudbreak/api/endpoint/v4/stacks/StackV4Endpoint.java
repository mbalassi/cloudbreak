...
public interface StackV4Endpoint {
    ...
    @PUT
    @Path("{name}/rotate_certificates")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = ROTATE_CERTIFICATES, nickname = "rotateCertificates")
    CertificatesRotationV4Response rotateCertificates(@PathParam("workspaceId") Long workspaceId, @PathParam("name") String name, @AccountId @QueryParam("accountId") String accountId,
            CertificatesRotationV4Request rotateCertificateRequest);
}
